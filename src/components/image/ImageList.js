...
class ImageList extends Component {
    ...
    renderImages() {
      ...
      const images = subdir?list.map(it => subdirUrl + '/' + it):[]
      const deleteButton = edit;
      return(
        subdir?
          <div>
            {this.state.deleted.length > 0?
              <button style={styles.button} onClick={this.deleteFiles}>Remove deleted files from disk</button>
            :null}
        ...
        :  
          <h2>No sub-directory in ImageList</h2>    
      )
    }                
